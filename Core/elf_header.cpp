...
{
    //check content size
	printf( "elf_header::from_string elfheader size : %d \n", str_content.size());
    if (str_content.size() < sizeof(Elf32_Ehdr)){
        printf("parse elf_header from string, but input string length invalid, len=%d \n", str_content.size());
        return false;
    }
...
bool elf_header::is_valid()
{
	...
	printf( "elf_header::is_valid header_.e_type : %d , ET_DYN : %d \n", header_.e_type, ET_DYN);
    if (header_.e_type != ET_DYN) {
        printf( "elf header has unexpected e_type: %d", header_.e_type);
        return false;
    }
	...
	printf( "elf_header::is_valid header_.e_machine : %d , ET_DYN : %d \n", header_.e_machine, GetTargetElfMachine());
