...
section_fix::~section_fix()
{
	...
	//printf(
bool section_fix::fix(std::string src_file, char* pc)
{
	printf("file_content in : %d , pp: %s \n", src_file.c_str(), pc);
	std::string filetest = std::string(pc);
	printf( "file_content 2 ftest : %s", filetest.c_str());
	std::string file_content = util::read_file(pc);
	printf("file_content : %d \n", file_content.length());
...
bool section_fix::save_as(std::string dst_file)
{
    ...
    {
        elf_section eee = (elf_section) itr;
		Elf32_Shdr &header_ = eee.get_header();
		if (header_.sh_type == SHT_DYNSYM) {
			printf("dynnnnnnnnnn size : %d \n", dynsymSize);
			section_table += m_sym_section.to_string();
			continue;
		}
	...
	/*
    std::sort(vec_load_.begin(), vec_load_.end(), [=](elf_segment &item1, elf_segment &item2)->bool{
	...
	printf("yush num before align : %d \n", sh_str_offset);
	int yush = (int)sh_str_offset % 4;
	if (yush > 0) {
		sh_str_offset = sh_str_offset + 4 - yush;
		char* em = (char*)malloc(yush);
		std::string emm = std::string(em);
		file_content_.append(em, yush);
		printf("yush num after align : %d \n", file_content_.size());
	}
...
bool section_fix::pre_load()
{
	...
	LOG(ERR, "[+] section_fix::pre_load [IN]");
    ...
    for (int idx = 0; idx < program_header_count; idx++){
        ...
        if (PT_LOAD == segment_.get_header().p_type){
			...
			if (meme == 0) {
				int firLoadSegSize = segment_.get_header().p_filesz;
				printf("first segment in meme : %d , fir : %d \n", meme, firLoadSegSize);
				vec_load_.push_back(segment_);
			}
            if (meme == 1) {
				secLoadSegSize = segment_.get_header().p_filesz;
				printf("second segment in meme : %d , sec : %d \n", meme, secLoadSegSize);
				vec_load_.push_back(segment_);
			}
			printf("first sdsss  segment on meme : %d \n", meme);
			meme += 1;
        }
...
//#define FILL_SECTION(dyn_tab, dyn_sz, type, flag, name)\
	...
	int dynstrOff = 0;
    dyn_item dyn_str_tabbb = dynamic_section_.find_dyn_by_tag(DT_STRTAB);
    dyn_item dyn_str_szzz = dynamic_section_.find_dyn_by_tag(DT_STRSZ);
    if (dyn_str_tabbb.is_valid() 
        && dyn_str_szzz.is_valid()){
        elf_section str_section_;
        Elf32_Shdr &header_ = str_section_.get_header();
        header_.sh_type = SHT_STRTAB;
        header_.sh_addr = dyn_str_tabbb.get_addr();
        header_.sh_offset = header_.sh_addr - calc_VA_FA_gap(header_.sh_addr);
		
		dynstrOff = header_.sh_offset;
    ...
    if (dyn_sym.is_valid()){
        ...
        header_.sh_type = SHT_DYNSYM;
        header_.sh_addr = dyn_sym.get_addr();
        header_.sh_entsize = dyn_sym_entry_size.get_value();
        header_.sh_offset = header_.sh_addr - calc_VA_FA_gap(header_.sh_addr);
        header_.sh_size = dynstrOff - header_.sh_offset;		// 待计算
        header_.sh_flags = SHF_ALLOC;
        header_.sh_name = find_string_idx_in_strtab(".dynsym");
		
        header_.sh_addralign = 4;
        header_.sh_link = 2; //very important, link means symname found base section, and now 1==.dynstr section index, because i put it in the second pos in the created_section_vec
		header_.sh_info = 1;

		dynsymSize = header_.sh_size;
		m_sym_section = sym_section_;
    ...
    if (dyn_hash.is_valid()){
		...
		bucketSize = hash_content_[0] + hash_content_[1] + 2;
		printf("hash 1 : %d ,  hash 2 : %d \n", hash_content_[0], hash_content_[2]);
        header_.sh_size = (hash_content_[0] + hash_content_[1] + 2) * sizeof(int);

        header_.sh_flags = SHF_ALLOC;
        header_.sh_addralign = 4;
		
		header_.sh_link = 1;	// 与 .dynsym 相关联
		header_.sh_entsize = 0x4;
		header_.sh_info = 0;
	...
	Elf32_Word plt_section_value = 0;
    {
		...
		plt_section_value = (dyn_rel_plt_sz.get_value()) / sizeof(Elf32_Rel);
    }
	...
	Elf32_Word fini_section_offs = 0;
    //create .fini_array
    ...
    if (dyn_fini_array.is_valid()
        ...
        header_.sh_offset = header_.sh_addr - 0x1000; // header_.sh_addr - calc_VA_FA_gap(header_.sh_addr);  为什么要减去 0x1000?需持续关注
		...
		fini_section_offs = header_.sh_offset;
    ...
    if (dyn_init_array.is_valid()
        ...
        header_.sh_offset = header_.sh_addr - 0x1000;		// 偏移需要修复，为什么是减去 0x1000
		//header_.sh_addr - calc_VA_FA_gap(header_.sh_addr);
	...
	Elf32_Addr dmc_section_addr = 0;
    Elf32_Word dmc_section_size = 0;
	Elf32_Word dmc_section_offs = 0;
    //create dynmamic section
    {
        ...
        header_.sh_flags = SHF_ALLOC | SHF_WRITE;	// 修改权限
        header_.sh_addralign = 4;
        header_.sh_link = 2;		// 与 .dynstr 相关联
        header_.sh_entsize = sizeof(Elf32_Dyn);
		
		dmc_section_addr = header_.sh_addr;
		dmc_section_size = header_.sh_size;
		dmc_section_offs = header_.sh_offset;
	...
	// create got section
	Elf32_Addr got_section_addr = 0;
    Elf32_Word got_section_size = 0;
	Elf32_Word got_section_offs = 0;
	{
        elf_section dyn_section_;
        Elf32_Shdr &header_ = dyn_section_.get_header();
		header_.sh_name = find_string_idx_in_strtab(".got");
		header_.sh_addr = dmc_section_addr + dmc_section_size;
		header_.sh_offset = dmc_section_offs + dmc_section_size;
		header_.sh_size = (bucketSize - 2) * 0x10 + 4; //plt_section_value * 0x10;		// 待修复
		header_.sh_type = SHT_PROGBITS;
		header_.sh_flags = SHF_ALLOC | SHF_WRITE;
		header_.sh_addralign = 8;
		header_.sh_entsize = 8;
		...
		got_section_addr = header_.sh_addr;
		got_section_size = header_.sh_size;
		got_section_offs = header_.sh_offset;
		
		vec_created_section_.push_back(dyn_section_);
		LOG(DBG, ".got section created");
	}
	...
	// create data section
	{
        elf_section dyn_section_;
		Elf32_Shdr &header_ = dyn_section_.get_header();
		header_.sh_name = find_string_idx_in_strtab(".data");
		header_.sh_addr = got_section_addr + got_section_size;
		header_.sh_offset = got_section_offs + got_section_size;
		header_.sh_size = fini_section_offs + secLoadSegSize - header_.sh_offset;	// 修复大小
		header_.sh_type = SHT_PROGBITS;
		header_.sh_flags = SHF_ALLOC | SHF_WRITE;
		header_.sh_addralign = 4;
		header_.sh_entsize = 0;
		header_.sh_link = 0;
		
		vec_created_section_.push_back(dyn_section_);
		LOG(DBG, ".data section created");
	}


    //fix symtab size
    return true;//fix_sym_tab_size() && fix_sym_item_section_ref();
}
...
int section_fix::find_string_idx_in_strtab(std::string str)
{
    if (sh_str_.empty()){
        ...
        //sh_str_ += (".ARM.butes" + null_seperator);	//attributes
        sh_str_ += (".dynamic" + null_seperator);
...
int section_fix::calc_addr_section_idx(Elf32_Off addr)
{
	...
	/*
    std::sort(vec_created_section_.begin(), vec_created_section_.end(),[=](elf_section &item1, elf_section &item2)->bool{
